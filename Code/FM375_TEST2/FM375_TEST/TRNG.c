...
INT32U hard_crc_calc(void)
{

	INT32U crc_result;
	
	CRCIF = 0;
	if(TRNGQIF & trng_busy)  //查询当前LFSR是否被占用
		return LFSR_BUSY;
	else
	{
		CRCRDATA0 = TX_Buf[3];
		CRCRDATA1 = TX_Buf[2];
		CRCRDATA2 = TX_Buf[1];
		CRCRDATA3 = TX_Buf[0];  //写入CRC输入数据
		
		CRCCTRL |= crc_en;     //启动CRC运算
		
		while(CRCCTRL & crc_en){}   //运算完成后，硬件自动清零crc_en
// 		while(!(CRCIF & crc_done)){}   //运算完成后，crc_done置起
// 		CRCIF &= ~crc_done;

		TX_Buf[7] = LFSROUT0;
		TX_Buf[8] = LFSROUT1;
		TX_Buf[9] = LFSROUT2;
		TX_Buf[10] = LFSROUT3;	//读取结果	
			
		crc_result = ((INT32U)TX_Buf[10]<<24) + ((INT32U)TX_Buf[9]<<16)	+ ((INT32U)TX_Buf[8]<<8) + TX_Buf[7];
