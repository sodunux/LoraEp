...
extern void delay(INT16U dlength) reentrant;  //由于中断中也可能使能此函数，所以定义成可重入函数
extern void delay_ms(INT32U dlength) reentrant;
extern void delay_us(INT16U dlength) reentrant;  //由于中断中也可能使能此函数，所以定义成可重入函数

extern void INIT_T0(void);
extern void INIT_T1(void);
extern unsigned char T2_capture_1ms_for_TRIM(unsigned char FREQ_mode,unsigned int trim_counter);
...
#define WDT_CLR()		do{PERICLK_CTRL2 |= wdt_clken;WDTSERV = 0x5A;}while(0)
